<template>
  <div class="home w-screen max-w-full h-screen bg-bodyColor grid grid-cols-12">
    <Sidebar class="col-start-1 col-end-4" />
    <div
      class="
        content
        col-start-1 col-end-13
        sm:col-start-2 sm:col-span-10
        lg:col-start-1 lg:col-end-10
        xl:col-start-4
        bmd:px-12
        h-screen
        flex
        justify-start
        items-center
        flex-col
      "
    >
      <div class="w-full xl:w-5/6 flex justify-center mt-6">
        <router-link to="/stories" class="w-1/4 sm:w-1/5 h-52 mr-2 rounded-xl overflow-hidden storyElement">
          <div class="addStoryImage bg-gray-400 h-4/6 rounded-t-xl"></div>
          <div class="relative bg-headerColor h-2/6 rounded-b-xl flex justify-center items-end">
            <div class="bg-headerColor w-10 h-10 rounded-full absolute -top-1/4 left-0 right-0 mx-auto flex justify-center items-center">
              <div class="bg-mainBlue w-8 h-8 rounded-full flex justify-center items-center">
                <font-awesome-icon icon="plus" color="white" />
              </div>
            </div>
            <p class="text-gray-300 text-sm mb-2 font-semibold">
              Utwórz <br />
              relację
            </p>
          </div>
        </router-link>

        <router-link to="/stories" class="w-1/4 sm:w-1/5 h-52 bg-gray-200 mr-2 rounded-xl overflow-hidden storyElement">
          <div class="addStoryImage bg-gray-400 h-full rounded-t-xl"></div>
        </router-link>
        <router-link to="/stories" class="w-1/4 sm:w-1/5 h-52 bg-gray-200 mr-2 rounded-xl overflow-hidden storyElement">
          <div class="addStoryImage bg-gray-400 h-full rounded-t-xl"></div>
        </router-link>
        <router-link to="/stories" class="w-1/4 sm:w-1/5 h-52 bg-gray-200 sm:mr-2 rounded-xl overflow-hidden storyElement">
          <div class="addStoryImage bg-gray-400 h-full rounded-t-xl"></div>
        </router-link>
        <router-link to="/stories" class="hidden sm:block w-1/5 h-52 bg-gray-200 rounded-xl overflow-hidden storyElement">
          <div class="addStoryImage bg-gray-400 h-full rounded-t-xl"></div>
        </router-link>
      </div>
      <div class="w-full sm:w-5/6 bg-gray-200 mt-6 rounded-lg h-40 p-2">
        <div>
          <img src="" alt="">
          <div v-if="postModal.postText == ''" @click="openPostModal">O czym myślisz, Dawid?</div>
          <div v-if="postModal.postText != ''">{{postModal.postText}}</div>
        </div>
        <div v-if="postModal.isOpen" class="absolute w-3/4 h-52 bg-mainBlue left-0 right-0 mx-auto">
          <input v-model="postModal.postText" type="text">
        </div>
        <div></div>
      </div>
      <div></div>
      <div></div>
    </div>
    <div></div>
  </div>
</template>

<script>
// @ is an alias to /src
import Sidebar from '@/components/Sidebar'

export default {
  name: 'Home',
  components: {
    Sidebar,
  },
  data() {
    return {
      postModal: {
        isOpen: false,
        postText: ''
      }
    }
  },
  methods: {
    openPostModal() {
      this.postModal.isOpen = true;
    }
  }
}
</script>

<style lang="scss" >
.addStoryImage {
  background-image: url('data:image/jpeg;base64,');
  background-size: cover;
  transition: transform 0.5s;
}
.storyElement {
  &:hover .addStoryImage {
    transform: scale(1.05);
  }
  &:hover {
    filter: brightness(90%);
  }
}
</style>
